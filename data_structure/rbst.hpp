...
public:
    struct Node {
        ...
        Node(const Tm& x) : sz(1), val(x), sum(x), lazy(MonoidOp::op_unit()) {}
...
private:
    ...
    static unique_ptr<Node> merge(unique_ptr<Node> nodeL, unique_ptr<Node> nodeR) noexcept {
        ...
        size_t sz_l = Node::size(nodeL), sz_r = Node::size(nodeR);
        if (randxor() % (sz_l + sz_r) < sz_l) {
            push(nodeL);
...
private:
    RBST(unique_ptr<Node>& node) : root(move(node)) {}

public:
    RBST() = default;
    RBST(size_t sz, const Tm& initial_value = MonoidOp::unit()) {
        vector<Tm> v(sz, initial_value);
        root = build(0, sz, v);
    }

    RBST(const vector<Tm>& v) {
    ...
    // [WARNING!!!]: Don't use this RBST after split
    inline pair< RBST, RBST > split(size_t pos) noexcept {
        auto p = split(move(root), pos);
        return {RBST(p.first), RBST(p.second)};
    }

    // merge tree from the right of this RBST
    // [WARNING!!!]: Don't use tree after merge
    inline void merge(RBST& tree) noexcept {
